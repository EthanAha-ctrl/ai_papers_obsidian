...
为了 build your intuition，我们从第一性原理 出发，将 Eigenvalue、QR algorithm 和 Lanczos algorithm 串联起来。核心直觉是：**Eigenvalue 揭示了线性变换的不变本质，QR 是通过正交剥离来逼近这个本质的迭代引擎，而 Lanczos 是在极端稀疏/海量维度下，利用 Krylov 子空间对 QR 的降维打击。**
...
### Reference Links:
1.  **QR Algorithm & Eigenvalues**: [Wikipedia: QR algorithm](https://en.wikipedia.org/wiki/QR_algorithm) (Details on shifts and convergence properties)
2.  **Lanczos Algorithm**: [Wikipedia: Lanczos algorithm](https://en.wikipedia.org/wiki/Lanczos_algorithm) (Details on loss of orthogonality and reorthogonalization)
3.  **Krylov Subspace Methods**: [Netlib: Templates for the Solution of Algebraic Eigenvalue Problems](https://www.netlib.org/utk/papers/templates/) (The authoritative technical resource for practical implementation)
4.  **First Principles of Linear Transformations**: [3Blue1Brown: Eigenvectors and Eigenvalues](https://www.3blue1brown.com/lessons/eigenvalues) (Visual intuition building)
---
为了 build your intuition，我们从第一性原理 出发来解构 **Krylov Subspace**。

核心直觉是：**Krylov Subspace 是 Matrix $A$ 施加在 Vector $b$ 上的“历史记忆累积空间”。它抛弃了对整个空间的盲目探索，转而利用 Matrix 的重复作用力，沿着最容易被激发的方向，低成本地重建出系统的核心动态。**
...
### Reference Links:
1.  **Krylov Subspace Definition & Polynomial View**: [Wikipedia: Krylov subspace](https://en.wikipedia.org/wiki/Krylov_subspace) (Core mathematical foundation)
2.  **Iterative Methods for Linear and Nonlinear Equations**: [SIAM Book by C. T. Kelley](https://www.siam.org/books/textbooks/fr16 Kelley.pdf) (Deep dive into Conjugate Gradient and GMRES based on Krylov)
3.  **Model Reduction and Moment Matching**: [PRIMA Algorithm Paper](https://www.cs.princeton.edu/courses/archive/fall06/cos526/papers/odabasioglu98.pdf) (Passive reduction for interconnect circuits using Krylov)
4.  **Mamba/SSM Connection**: [Mamba: Linear-Time Sequence Modeling with Selective State Spaces](https://arxiv.org/abs/2312.00752) (Relating recursive state evolution to matrix powers)