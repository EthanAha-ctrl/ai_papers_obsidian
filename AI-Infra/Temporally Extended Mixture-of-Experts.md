...
标准 MoE 的做法是:**每写一个字,都重新挑一次书架**。写"今天"去书架 A,写"天气"又跑去书架 B,写"很好"又换到书架 C。光跑来跑去就累死了,真正写作业的时间没多少。

这论文说:别这么折腾啊!**既然你在做一道数学题,接下来几十个字大概率都还是数学,你就在数学书架前蹲着别动,真要换领域了再换**。

这就是所谓的 "temporally extended"——一次决策管好一段路,而不是每个字都重新纠结。

## 为什么这事突然重要了

以前没人觉得"每 token 换专家"是个问题,因为大家都假设 GPU 内存够大,32 个专家全驻在里面,切换就是改个指针,零成本。

但现在模型越搞越大,128 个专家、512 个专家、甚至百万个专家……GPU 装不下了,得把一部分专家赶出 GPU,存到 CPU 内存甚至硬盘上。要用的时候再 fetch 进来。

这时候 "每 token 切一次" 就变成灾难了:你每个字都要去硬盘捞一次专家,延迟爆炸。

作者测了一下当前几个开源大模型,**切换率高达 94%-100%**,基本就是每个 token 都在换。Figure 2 那张图看得特别清楚,像噪点一样乱。
