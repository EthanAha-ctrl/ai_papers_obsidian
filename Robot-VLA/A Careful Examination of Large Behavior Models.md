...
# TRI Large Behavior Models Paper 深度解析

## 2. 方法论：Diffusion Policy + DiT

### 2.2 网络架构

- **Vision encoder**: CLIP ViT-B/16，取 CLS token 输出，训练时 fine-tune
- **Language encoder**: CLIP text encoder，frozen，但上面加一个可训练的 projection layer
- **Proprioception(本体感觉)**: end-effector pose（相对 station base frame 和相对另一只手）、gripper width
- **Diffusion timestep embedding**: sinusoidal PE + 2-layer MLP，通过 adaptive LayerNorm (adaLN) 注入
- **DiT 主体**: 8 个 DiT block，n_dim 768
...
- **TRI-Ramen-UMI**: 32 小时，129 tasks，10,851 demos
...
**主要发现 1**: Finetuned Large Behavior Models 在 aggregate 上统计显著优于 single-task baseline，在 nominal 和 distribution shift 条件下均成立。

具体数字（sim 16 个 tasks）：
- Nominal: finetuned Large Behavior Models 在 3/16 tasks 显著优于 single-task，剩余 13/16 不可区分
- Distribution shift: finetuned Large Behavior Models 在 10/16 tasks 显著优于 single-task

**主要发现 2**: Pretrained-only Large Behavior Models（不 finetune）在 seen tasks 上 success rate > 0，aggregate 上与 single-task 在 sim 中不可区分，但在 real-world 上略差。作者归因于：
1. Real-world 任务更难
...
**主要发现 3**: Finetuned Large Behavior Models 在 task completion 上统计显著优于 single-task baseline（4/5 real-world tasks，4/5 sim tasks）。Success rate 上虽然数字低，但在能分离的 tasks 上都是 Large Behavior Models 胜出。

**主要发现 4 — Data Efficiency**: Large Behavior Models finetune 只用 15% 数据，就统计显著优于用 100% 数据训练的 single-task baseline（SetBreakfastTable, Fig. 5）。Aggregate sim 数据显示 Large Behavior Models 达到 single-task baseline 等价性能只需 < 30% 数据，即 **3-5x data efficiency**。
...
Section X 和 Fig. S13 揭示了一个微妙现象：pretrained Large Behavior Models 和 finetuned Large Behavior Models 在 simulation 中常常 rollout 开始后久久不动（甚至 timeout）。但 single-task policy 没这个问题。更奇怪的是，在 distribution shift 条件下，Large Behavior Models 反而更快开始动。

作者的 hypothesis：低 motion frames 在 pretraining data 中比例较高（sim 11.7%, real 3.3%）。如果 pretraining 时 filter 掉这些 frames，Large Behavior Models 启动更快，但语言可 steer 能力下降——会 commit 到错误任务。这个 tradeoff 暴露了 multitask model 的一个根本张力：**等待 language 信号 disambiguate 与及时行动之间的权衡**。
...
Large Behavior Models 的 finetune superiority over single-task training，与 LLM 里 "pretrain-then-finetune" 优于 "train from scratch" 完全对应。但有一个重要差异：**robotics data 的 marginal cost 远高于 text data**。1,700 小时 demonstration 在 robotics 算大规模，但对应 token 数远不及 LLM 的 pretraining corpus。这意味着 Large Behavior Models 的 scaling curve 还远未饱和，未来的瓶颈在 data collection infrastructure。
...
TRI 的 Large Behavior Models 在架构上更保守（DiT + CLIP，没有大规模 LLM backbone），但 evaluation 上更严谨。

### 8.2 Diffusion vs. Autoregressive Token Prediction

Large Behavior Models 选择 diffusion 而非 tokenized action 的 autoregressive prediction（如 RT-2, OpenVLA 的做法）。Diffusion 的优势在于：
- 连续 action space 的精度
- Multi-modality 自然支持
- 不需要 action tokenizer 设计

但代价是 inference 速度（多次 denoising step）。Large Behavior Models 在 10 Hz 运行，已经能满足 bimanual tabletop manipulation 的需求。这个 tradeoff 在 Brooks et al. (https://arxiv.org/abs/2410.24164) 的 π0 里用 flow matching 部分缓解。
...
Large Behavior Models 的 sim-real performance reversal 提醒我们：sim 的"完美可控"也是它的局限——operator judgment 包含大量 implicit domain knowledge。
...
1. **Continual learning**: Large Behavior Models finetune 后是否会 catastrophic forget 其他 task？未测
...
4. **Causal understanding**: Large Behavior Models 是否有 object affordance 的隐式 model，还是纯 pattern matching？
