...
这篇 paper 就是想搞清楚一件事：**自动驾驶系统从"感知-决策-控制"的老路子，怎么慢慢演变成"看图+读文字+做推理+输出动作"这种更像人的开车方式**，作者把这个新范式叫 **VLA (Vision-Language-Action)**。

---

## 为什么要搞 VLA？老的 VA 有啥问题？

老的 VA (Vision-Action) model 就是直接把摄像头画面映射成方向盘角度、油门刹车这种 control signal。听起来很直接，但实际有几个硬伤：

**第一个问题：它是黑盒。** 你不知道它为啥这么开。出了事故没法复盘——是没看见行人？看见了但判断错了？判断对了但执行错了？VA model 给不了你答案。

**第二个问题：它不会推理。** 你给它一张图，它直接吐 trajectory，中间没有"思考"过程。遇到十字路口四辆车同时到，谁先走？遇到救护车要不要让？这种需要 chain-of-thought 的场景，纯 VA model 本质上是在 pattern matching，不是在 reasoning。

**第三个问题：它听不懂人话。** 你没法跟它说"开稳一点"、"别走那条路"、"前面小心有施工"。人车交互完全断掉了。

VLA 就是想解决这三个问题——**让模型能解释自己为啥这么开，遇到复杂场景能推理，还能听懂乘客的指令**。
