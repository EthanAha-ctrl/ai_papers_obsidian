**Time Delay and Integration (TDI)** 是一种**前向运动补偿** 技术，专门用于在低光照条件下捕获运动物体的图像。它是一种特殊的**线扫描** 成像模式，通过将多个线性阵列并排放置，并同步电荷转移与物体运动，实现对同一目标的多重曝光叠加。TDI 的本质是**构造性干涉**——将多次独立观测的结果叠加，从而在不牺牲空间分辨率的情况下提高信噪比。这与天文观测中的长时间曝光、斑点成像 和自适应光学 的原理相通。
